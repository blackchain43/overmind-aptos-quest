...
module overmind::breeder_core {
    ...
    */
    public entry fun init(admin: &signer) {
        ...
        let (resource_account, resource_account_cap) = account::create_resource_account(admin, BREEDER_SEED);
        ...
        move_to(
            ...
            State {
                breeder: Breeder{
                    collections: simple_map::create<String, DragonRace>(),
                    ongoing_breedings: simple_map::create<vector<u8>, u64>(),
                    create_dragon_collection_events: account::new_event_handle<CreateDragonCollectionEvent>(&resource_account),
                    create_dragon_events: account::new_event_handle<CreateDragonEvent>(&resource_account),
                    breed_dragons_events: account::new_event_handle<BreedDragonsEvent>(&resource_account),
                    hatch_dragon_events: account::new_event_handle<HatchDragonEvent>(&resource_account)
                },
                combiner: Combiner{
                    collections: simple_map::create<String, SwordType>(),
                    create_sword_collection_events: account::new_event_handle<CreateSwordCollectionEvent>(&resource_account),
                    create_sword_events: account::new_event_handle<CreateSwordEvent>(&resource_account),
                    combine_swords_events: account::new_event_handle<CombineSwordsEvent>(&resource_account)
                },
    ...
    */
    public entry fun create_dragon_collection(
        ...
        //  Assert that breeding time is correct
        ...
        let (collection_names, _) = simple_map::to_vec_pair(state.breeder.collections);
        assert_collection_does_not_exist(&collection_names, &name);

        //  Create a collection
        create_collection_internal(&resource_account_signer, name, description, uri, DRAGON_COLLECTION_MAX_SUPPLY, false, true);

        //  Calculate monster starting properties
        let dragon_properties = calculate_dragons_starting_properties(breeding_time);
        let starting_properties = vector::map_ref(&dragon_properties, |value| {
          bcs::to_bytes(value)
        });

        //  Push ability property to the starting properties
        vector::push_back(&mut starting_properties, bcs::to_bytes(&ability_property));

        //  Add a new MonsterRace to Breeder's collections
        simple_map::add(&mut state.breeder.collections, name, DragonRace { breeding_time, starting_properties });


        //  Emit CreateMonsterCollectionEvent event
        event::emit_event<CreateDragonCollectionEvent>(
            &mut state.breeder.create_dragon_collection_events,
            breeder_events::new_create_dragon_collection_event(
                name,
                description,
                uri,
                breeding_time,
    ...
    */
    public entry fun create_sword_collection(
        ...
        //  Assert that combine amount is correct
        assert_combine_amount_is_correct(combine_amount);

        //  Calculate equipment starting properties sum
        let starting_properties_sum = get_equipment_starting_properties_sum(combine_amount);

        //  Assert that sum of provided property_values is correct
        assert_sword_property_values_sum_is_correct(&property_values, starting_properties_sum);

        //  Assert that state is initialized
        assert_state_initialized();

        //  Assert that collection with provided name does not exist
        let state = borrow_global_mut<State>(@admin);
        let resource_account_signer = account::create_signer_with_capability(&state.cap);
        let (collection_names, _) = simple_map::to_vec_pair(state.combiner.collections);
        assert_collection_does_not_exist(&collection_names, &name);

        //  Create a collection
        create_collection_internal(&resource_account_signer, name, description, uri, SWORD_COLLECTION_MAX_SUPPLY, true, false);

        //  Create a new Equipment and add it to Combiner's collections
        let starting_properties = vector::map_ref(&property_values, |value| {
          bcs::to_bytes(value)
        });
        vector::push_back(&mut starting_properties, bcs::to_bytes(&ability_property));
        simple_map::add(&mut state.combiner.collections, name, SwordType { combine_amount, starting_properties });

        //  Emit CreateEquipmentCollectionEvent event
    ...
    */
    public entry fun create_dragon(
        ...
        //  Assert that collection with provided name exists
        let state = borrow_global_mut<State>(@admin);
        let resource_account_signer = account::create_signer_with_capability(&state.cap);
        let resource_account_address =  signer::address_of(&resource_account_signer);
        let (collection_names, _) = simple_map::to_vec_pair(state.breeder.collections);
        assert_collection_exists(&collection_names, &collection_name);

        //  Create a variable holding PDA's GUID next creation number
        let next_creation_number = account::get_guid_next_creation_num(resource_account_address);

        //  Mint a new NFT
        let account_address = signer::address_of(account);
        let dragon_prop_keys = get_property_params_as_strings(&DRAGON_PROPERTY_KEYS);
        let dragon_prop_types = get_property_params_as_strings(&DRAGON_PROPERTY_TYPES);
        let starting_properties = simple_map::borrow(&state.breeder.collections, &collection_name).starting_properties;
        aptos_token::mint(
        ...
        let dragon_token_address = object::create_guid_object_address(resource_account_address, next_creation_number);
        object::transfer_raw(&resource_account_signer, dragon_token_address, account_address);
        //  Emit CreateMonsterEvent event
    ...
    */
    public entry fun create_sword(
        ...
        //  For every token to be created:
        //      1. Create a variable holding PDA's GUID next creation number
        //      2. Mint a new NFT
        //      3. Transfer the NFT to the signer of the transaction
        let sword_prop_keys = get_property_params_as_strings(&SWORD_PROPERTY_KEYS);
        let sword_prop_types = get_property_params_as_strings(&SWORD_PROPERTY_TYPES);
        let starting_properties = simple_map::borrow(&state.combiner.collections, &collection_name).starting_properties;
        let creation_numbers = vector::empty<u64>();
        let account_address = signer::address_of(account);
        let i = 0;
        while(i < amount){
            let next_creation_number = account::get_guid_next_creation_num(signer::address_of(&resource_account_signer));
            // let royalty = royalty::create(1, 10, signer::address_of(&resource_account_signer));
            aptos_token::mint(
                &resource_account_signer,
                collection_name,
                sword_description,
            ...
            //  Transfer the NFT to the signer of the transaction
            let sword_token_address = object::create_guid_object_address(resource_account_address, next_creation_number);
            object::transfer_raw(&resource_account_signer, sword_token_address, account_address);
            vector::push_back(&mut creation_numbers, next_creation_number);
            i = i + 1;
        };

        //  Emit CreateEquipmentEvent event
    ...
    */
    public entry fun breed_dragons(
        ...
        //  Assert that the signer owns the first monster token
        let resource_account_address = signer::address_of(&resource_account_signer);
        let first_dragon_address = object::create_guid_object_address(resource_account_address, first_dragon_creation_number);
        let first_dragon_token = object::address_to_object<Token>(first_dragon_address);
        assert_signer_owns_token(owner, first_dragon_token);

        //  Assert that the first monster token is from the provided collection
        assert_token_is_from_correct_collection(collection_name, first_dragon_token);

        //  Assert that the first monster is not breeding
        assert_dragon_not_breeding(first_dragon_token);

        //  Assert that the signer owns the second monster token
        let second_dragon_address = object::create_guid_object_address(resource_account_address, second_dragon_creation_number);
        let second_dragon_token = object::address_to_object<Token>(second_dragon_address);
        assert_signer_owns_token(owner, second_dragon_token);

        //  Assert that the second monster token is from the provided collection
        assert_token_is_from_correct_collection(collection_name, second_dragon_token);

        //  Assert that the second monster is not breeding
        assert_dragon_not_breeding(second_dragon_token);

        //  Create a hash from both of monster addresses
        ...
        //  Add new record to Breeder's ongoing_breedings
        let breeding_time = simple_map::borrow(&state.breeder.collections, &collection_name).breeding_time;
        simple_map::add(&mut state.breeder.ongoing_breedings, breeding_key, breeding_time);

        //  Freeze transfer of both tokens
        aptos_token::freeze_transfer(&resource_account_signer, first_dragon_token);
        aptos_token::freeze_transfer(&resource_account_signer, second_dragon_token);

        //  Emit BreedMonsterEvent event
    ...
    */
    public entry fun hatch_dragon(
        ...
        //  Assert that the monsters are breeding
        let breeding_key_bytes = bcs::to_bytes(&first_dragon_address);
        vector::append(&mut breeding_key_bytes, bcs::to_bytes(&second_dragon_address));
        let breeding_key = aptos_hash::sha3_512(breeding_key_bytes);
        assert_dragons_are_breeding(&state.breeder.ongoing_breedings, &breeding_key);

        //  Assert that the breeding has finished
        assert_breeding_finished(&state.breeder.ongoing_breedings, &breeding_key);

        //  Remove breeding record from Breeder's ongoing_breedings
        simple_map::remove(&mut state.breeder.ongoing_breedings, &breeding_key);

        //  Unfreeze transfer of both monster tokens
        aptos_token::unfreeze_transfer(&resource_account_signer, first_dragon_token);
        aptos_token::unfreeze_transfer(&resource_account_signer, second_dragon_token);

        //  Create a variable and save PDA's GUID next creation number
        let next_creation_number = account::get_guid_next_creation_num(resource_account_address);

        //  Combine properties of the monster tokens
        let dragon_prop_keys = get_property_params_as_strings(&DRAGON_PROPERTY_KEYS);
        let dragon_prop_types = get_property_params_as_strings(&DRAGON_PROPERTY_TYPES);
        let new_dragon_properties = combine_properties(
            dragon_prop_keys,
            vector[
                object::address_to_object<PropertyMap>(first_dragon_address),
                object::address_to_object<PropertyMap>(second_dragon_address)
            ]
        );

        //  Mint a new monster token
        aptos_token::mint(
            &resource_account_signer,
            token::collection_name(first_dragon_token),
            new_dragon_description,
            new_dragon_name,
            new_dragon_uri,
            dragon_prop_keys,
            dragon_prop_types,
            new_dragon_properties
        );


        //  Transfer the NFT to the signer of the transaction
        let new_dragon_token_address = object::create_guid_object_address(resource_account_address, next_creation_number);
        object::transfer_raw(&resource_account_signer, new_dragon_token_address, owner_address);



        //  Emit HatchMonsterEvent event
    ...
    */
    public entry fun combine_swords(
        ...
        //  Assert that amount of equipment to burn is correct
        let state = borrow_global_mut<State>(@admin);
        let resource_account_signer = account::create_signer_with_capability(&state.cap);
        let resource_account_address = signer::address_of(&resource_account_signer);
        let combine_amount = simple_map::borrow(&state.combiner.collections, &collection_name).combine_amount;
        assert_amount_of_swords_is_correct(&swords_creation_numbers, combine_amount);

        //  Create a vector and for each of equipment's creation numbers:
        //      1. Create Object<PropertyMap>
        //      2. Push it to the vector
        let property_maps = vector::empty<Object<PropertyMap>>();
        let sword_addresses = vector::empty<address>();
        vector::for_each_ref(&swords_creation_numbers, |num| {
            let num: &u64 = num;
            let sword_address = object::create_guid_object_address(resource_account_address, *num);
            let sword_token_property = object::address_to_object<PropertyMap>(sword_address);
            vector::push_back(&mut property_maps, sword_token_property);
            vector::push_back(&mut sword_addresses, sword_address);
        });

        //  Combine properties of the equipment
        let sword_prop_keys = get_property_params_as_strings(&SWORD_PROPERTY_KEYS);
        let sword_prop_types = get_property_params_as_strings(&SWORD_PROPERTY_TYPES);
        let new_sword_properties = combine_properties(
            sword_prop_keys,
            property_maps
        );

        //  For each of equipement's creation numbers
        //      1. Assert that the signer owns the token
        //      2. Assert that the token is from the provided collection
        //      3. Burn the token
        vector::for_each_ref(&sword_addresses, |addr| {
            let addr: &address = addr;
            // let sword_address = object::create_guid_object_address(resource_account_address, *num);
            let sword_token = object::address_to_object<Token>(*addr);
            assert_signer_owns_token(owner, sword_token);
            assert_token_is_from_correct_collection(collection_name, sword_token);
            aptos_token::burn(&resource_account_signer, sword_token);
        });

        //  Save PDA's GUID next creation number
        let next_creation_number = account::get_guid_next_creation_num(resource_account_address);

        //  Mint a new equipment NFT with combined properties
        // let royalty = royalty::create(1, 10, signer::address_of(&resource_account_signer));
        ...
        aptos_token::mint(
            ...
            sword_prop_keys,
            sword_prop_types,
            new_sword_properties
        );

        //  Transfer the NFT to the signer of the transaction
        let new_sword_token_address = object::create_guid_object_address(resource_account_address, next_creation_number);
        object::transfer_raw(&resource_account_signer, new_sword_token_address, owner_address);


        //  Emit CombineEquipmentEvent event
    ...
    */
    ...
    public fun get_equipment_starting_properties_sum(combine_amount: u64): u64 {
        //  Assert that provided combine amount is correct
        assert_combine_amount_is_correct(combine_amount);

        //  Calcualte and return equipment starting properties sum
    ...
    */
    inline fun create_collection_internal(
        ...
        //  Call aptos_token::creation_collection function with appropriate parameters
    ...
    */
    inline fun get_property_params_as_strings(property_params: &vector<vector<u8>>): vector<String> {
        //  Convert vector of byte representations into vector of string representations
        vector::map_ref(property_params,|param|{
            string::utf8(*param)
        })
    }
    ...
    */
    inline fun calculate_dragons_starting_properties(breeding_time: u64): vector<u64> {
        //  Calculate monster starting properties accordingly to the formula:
        //        (b_t - MIN_B_T)^3
        //      --------------------- * P_Diff + P_Min
        //      (MAX_B_T - MIN_B_T)^3
        // Where:
        ...
        let numerator = math64::pow(breeding_time - MINIMAL_BREEDING_TIME, 3);
        let denumerator = math64::pow(MAXIMAL_BREEDING_TIME - MINIMAL_BREEDING_TIME, 3);
        let result = vector::empty<u64>();
        let i = 0;
        while(i < vector::length(&DRAGON_MINIMAL_START_PROPERTY_VALUES)){
            let p_diff = *vector::borrow(&DRAGON_MAXIMAL_START_PROPERTY_VALUES, i) - *vector::borrow(&DRAGON_MINIMAL_START_PROPERTY_VALUES, i);
            let p_min = *vector::borrow(&DRAGON_MINIMAL_START_PROPERTY_VALUES, i);
            vector::push_back(&mut result, math64::mul_div(numerator, p_diff, denumerator) + p_min);
            i = i + 1;
        };
        result
    }
    ...
    */
    inline fun calculate_swords_starting_properties_sum(combine_amount: u64): u64 {
        //  Calculate sum of equipment starting properties accordingly to the formula:
        //          (c_a - MIN_AMOUNT)^2
        //      --------------------------- * P_Diff + P_MIN
        ...
        let numerator = math64::pow(combine_amount - MINIMAL_AMOUNT_TO_COMBINE, 2);
        let denumerator = math64::pow(MAXIMAL_AMOUNT_TO_COMBINE - MINIMAL_AMOUNT_TO_COMBINE, 2);
        math64::mul_div(numerator, SWORD_MAXIMAL_START_PROPERTY_VALUES_SUM - SWORD_MINIMAL_START_PROPERTY_VALUES_SUM, denumerator) + SWORD_MINIMAL_START_PROPERTY_VALUES_SUM
    }
    ...
    */
    inline fun combine_properties(
        ...
        //  Create a vector for combined properties
        let combine_properties = vector::empty<vector<u8>>();

        //  For each of property keys:
        ...
        vector::for_each_ref(&property_keys, |key|{
            let key: &String = key;
            let acc = 0;
            let i = 0;
            while(i < vector::length(&property_maps)) {
                let prop_map = vector::borrow(&property_maps, i);
                let (type, value) = property_map::read<PropertyMap>(prop_map, key);
                if(type == string::utf8(b"u64")){
                    acc = acc + from_bcs::to_u64(value);
                } else {
                    vector::push_back(&mut combine_properties, value);
                    break
                };
                i = i + 1;
            };
            if(acc != 0){
                vector::push_back(&mut combine_properties, bcs::to_bytes(&acc));
            };
        });
        combine_properties
    }
    ...
    inline fun assert_breeding_time_is_correct(breeding_time: u64) {
        //  Assert that breeding_time is greater or equals to MINIMAL_BREEDING_TIME and is smaller or equals to
    ...
    inline fun assert_combine_amount_is_correct(combine_amount: u64) {
        //  Assert that combine_amount is greater or equals to MINIMAL_TO_COMBINE and is smaller
    ...
    inline fun assert_sword_property_values_sum_is_correct(property_values: &vector<u64>, expected_sum: u64) {
        //  Assert that sum of property_values' values is smaller or equals expected_sum
        assert!(vector::fold(*property_values, 0, |a, b| a + b) <= expected_sum, ERROR_INVALID_SWORDS_PROPERTY_VALUES_SUM);
    }

    inline fun assert_collection_does_not_exist(collections: &vector<String>, collection_name: &String) {
        //  Assert that the vector does not contain the collection's name
    ...
    inline fun assert_collection_exists(collections: &vector<String>, collection_name: &String) {
        //  Assert that the vector contains the collection's name
    ...
    inline fun assert_signer_owns_token(owner: &signer, token: Object<Token>) {
        //  Assert that address of the owner is the same as the owner of the object
        assert!(object::is_owner<Token>(token, signer::address_of(owner)), ERROR_SIGNER_IS_NOT_THE_OWNER);
    }

    inline fun assert_dragon_not_breeding(monster: Object<Token>) {
        //  Assert that transfer of the object is allowed
        assert!(object::ungated_transfer_allowed<Token>(monster), ERROR_DRAGON_DURING_BREEDING);
    }
    ...
    inline fun assert_dragons_are_breeding(
        ...
        //  Assert that the map contains the provided key
    ...
    inline fun assert_breeding_finished(
        ...
        //  Assert that timestamp related to the provided monster_pair_hash is smaller or equals current timestamp
        assert!(*simple_map::borrow(ongoing_breedings, dragons_pair_hash) <= timestamp::now_seconds(), ERROR_BREEDING_HAS_NOT_ENDED);
    }

    inline fun assert_amount_of_swords_is_correct(swords: &vector<u64>, combine_amount: u64) {
        //  Assert that the vector's length equals to combine_amount
    ...
    inline fun assert_property_vectors_lengths(
        ...
        //  Assert that each of the property maps length equals to number of keys in property_keys
        let property_keys_len = vector::length(property_keys);
        let i = 0;
        while(i < vector::length(properties)){
            assert!(property_map::length<PropertyMap>(vector::borrow(properties, i)) == property_keys_len, ERROR_PROPERTY_LENGTH_MISMATCH);
            i = i + 1;
        };
        // vector::for_each_ref(properties, |map|{
        //     let map: &Object<PropertyMap> = map;
        //     assert!(property_map::length<PropertyMap>(map) == property_keys_len, ERROR_PROPERTY_LENGTH_MISMATCH);
        // });
    }

    inline fun assert_token_is_from_correct_collection(collection_name: String, token: Object<Token>) {
        //  Assert that collection of the token is the same as collection_name
        assert!(token::collection_name<Token>(token) == collection_name, ERROR_TOKEN_FROM_WRONG_COLLECTION);
    }
    ///////////
    // TESTS //
  ///////////
