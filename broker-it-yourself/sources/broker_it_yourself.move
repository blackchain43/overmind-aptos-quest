...
module overmind::broker_it_yourself {
    ...
    use overmind::broker_it_yourself_events::{CreateOfferEvent, AcceptOfferEvent, CompleteTransactionEvent, ReleaseFundsEvent, CancelOfferEvent, OpenDisputeEvent, ResolveDisputeEvent, Self};
    ...
    */
    public entry fun init(admin: &signer) {
        //  Call assert_signer_is_admin function
        assert_signer_is_admin(admin);

        //  Create a resource account using `SEED` global constant
        let (pda_account, cap) = account::create_resource_account(admin, SEED);

        //  Register the resource account with AptosCoin
        coin::register<AptosCoin>(&pda_account);
        ...
        move_to(
            ...
            State {
                ...
                offer_id: 0,
                cap,
                create_offer_events: account::new_event_handle<CreateOfferEvent>(&pda_account),
                accept_offer_events: account::new_event_handle<AcceptOfferEvent>(&pda_account),
                complete_transaction_events: account::new_event_handle<CompleteTransactionEvent>(&pda_account),
                release_funds_events: account::new_event_handle<ReleaseFundsEvent>(&pda_account),
                cancel_offer_events: account::new_event_handle<CancelOfferEvent>(&pda_account),
                open_dispute_events: account::new_event_handle<OpenDisputeEvent>(&pda_account),
                resolve_dispute_events: account::new_event_handle<ResolveDisputeEvent>(&pda_account)
            }
        );
    }
    ...
    */
    public entry fun create_offer(
        ...
        let next_offer = get_next_offer_id(&mut state.offer_id);
        ...
        simple_map::add(&mut state.offers, next_offer, offer);

        //  Add the offer id to the creator's offers list
        if(simple_map::contains_key(&state.creators_offers, &creator_addr)) {
            vector::push_back(simple_map::borrow_mut(&mut state.creators_offers, &creator_addr), next_offer);
        } else {
            simple_map::add(&mut state.creators_offers, creator_addr, vector<u128>[next_offer]);
        };
        ...
        if(sell_apt){
            ...
            coin::transfer<AptosCoin>(creator, signer::address_of(&account::create_signer_with_capability(&state.cap)), apt_amount);
        };
    ...
    */
    public entry fun accept_offer(user: &signer, offer_id: u128) acquires State {
        ...
        let party = signer::address_of(user);
        offer.counterparty = option::some(party);

        //  Transfer appropriate APT amount from the user to the PDA if Offer's sell_apt == false &&
        //      assert_user_has_enough_funds
        if(!offer.sell_apt){
            assert_user_has_enough_funds<AptosCoin>(party, offer.apt_amount);
            coin::transfer<AptosCoin>(user, signer::address_of(&account::create_signer_with_capability(&state.cap)), offer.apt_amount);
        };

        //  Emit AcceptOfferEvent event
        event::emit_event<AcceptOfferEvent>(
            &mut state.accept_offer_events,
            broker_it_yourself_events::new_accept_offer_event(
                offer_id,
                party,
                timestamp::now_microseconds()
            )
    ...
    */
    public entry fun complete_transaction(user: &signer, offer_id: u128) acquires State {
        ...
        let party = signer::address_of(user);
        assert_user_participates_in_transaction(party, offer);

        //  call assert_user_has_not_marked_completed_yet function
        assert_user_has_not_marked_completed_yet(party, offer);

        //  call assert_dispute_not_opened function
        assert_dispute_not_opened(offer);

        //  Compare the user's address and set appropriate completion flag to true
        if (party == offer.creator) {
            offer.completion.creator = true;
        } else if (option::contains(&offer.counterparty, &party)){
        ...
        if(offer.completion.creator && offer.completion.counterparty){
            ...
            if(offer.sell_apt){
                coin::transfer<AptosCoin>(
                    ...
                    *option::borrow(&offer.counterparty), 
                    offer.apt_amount
                );
            } else {
                coin::transfer<AptosCoin>(
    ...
    */
    public entry fun cancel_offer(creator: &signer, offer_id: u128) acquires State {
        ...
        let (_, offer) = simple_map::remove(&mut state.offers, &offer_id);
    ...
    */
    public entry fun resolve_dispute(
        ...
        assert_dispute_opened(simple_map::borrow(&state.offers, &offer_id));

        //  Call assert_singer_is_arbiter function
        assert_singer_is_arbiter(arbiter, simple_map::borrow(&state.offers, &offer_id));
    ...
    */
    ...
    public fun get_available_offers(): SimpleMap<u128, Offer> acquires State {
        ...
        let available_offers = simple_map::create<u128, Offer>();
        let state = borrow_global<State>(@admin);
        ...
        while(i < vector::length(&values)){
            ...
            if(option::is_none(&offer.counterparty)){
                ...
                simple_map::add(&mut available_offers, offer_id, *offer);
            };
            i = i + 1;
        };
        available_offers
    ...
    */
    ...
    public fun get_arbitration_offers(): SimpleMap<u128, Offer> acquires State {
        ...
        //  Returns a list of the offers that have dispute opened
        let arbitration_offers = simple_map::create<u128, Offer>();
        let state = borrow_global<State>(@admin);
        let (keys, values) = simple_map::to_vec_pair(state.offers);
        let i = 0;
        while(i < vector::length(&values)){
            ...
            if(offer.dispute_opened){
                let offer_id = *vector::borrow(&keys, i);
                simple_map::add(&mut arbitration_offers, offer_id, *offer);
            };
            i = i + 1;
        };
        arbitration_offers
    ...
    */
    ...
    public fun get_buy_offers(creator: address): SimpleMap<u128, Offer> acquires State {
        ...
        let state = borrow_global<State>(@admin);
        let creator_offer_list = simple_map::borrow(&state.creators_offers, &creator);
        let i = 0;
        while(i < vector::length(creator_offer_list)){
            let offer_id = vector::borrow(creator_offer_list, i);
            let offer = simple_map::borrow(&state.offers, offer_id);
            if(!offer.sell_apt){
                simple_map::add(&mut buy_offers, *offer_id, *offer);
            };
            i = i + 1;
        };
        buy_offers
    ...
    */
    ...
    public fun get_sell_offers(creator: address): SimpleMap<u128, Offer> acquires State {
        ...
        while(i < vector::length(creator_offer_list)){
            ...
            let offer = simple_map::borrow(&state.offers, offer_id);
            if(offer.sell_apt){
                simple_map::add(&mut sell_offers, *offer_id, *offer);
            };
            i = i + 1;
        };
        sell_offers
    ...
    */
    ...
    public fun get_creator_offers(creator: address): SimpleMap<u128, Offer> acquires State {
        ...
        while(i < vector::length(creator_offer_list)){
            let offer_id = *vector::borrow(creator_offer_list, i);
            let offer = *simple_map::borrow(&state.offers, &offer_id);
            simple_map::add(&mut creator_offers, offer_id, offer);
            i = i + 1;
        };
        creator_offers
    }
    ...
    */
    public(friend) inline fun remove_offer_from_creator_offers(
        ...
        let creator_offer_list = simple_map::borrow_mut(creators_offers, creator);
        let (_, index) = vector::index_of(creator_offer_list, offer_id);
        vector::remove(creator_offer_list, index);
    }
    ...
    */
    ...
    inline fun assert_offer_exists(
        ...
        assert!(
            simple_map::contains_key(offers, offer_id), 
            ERROR_OFFER_DOES_NOT_EXIST
        );
    }
    ...
    inline fun assert_offer_not_accepted(offer: &Offer) {
        ...
        assert!(
            option::is_none(&offer.counterparty), 
            ERROR_OFFER_ALREADY_ACCEPTED
        );
    }
    ...
    inline fun assert_offer_accepted(offer: &Offer) {
        ...
        assert!(
            option::is_some(&offer.counterparty), 
            ERROR_OFFER_NOT_ACCEPTED
        );
    }
    ...
    inline fun assert_user_participates_in_transaction(user: address, offer: &Offer) {
        ...
        assert!(
            offer.creator == user || 
            option::contains(&offer.counterparty, &user), 
    ...
    inline fun assert_signer_is_creator(creator: &signer, offer: &Offer) {
        ...
        assert!(
            signer::address_of(creator) == offer.creator, 
            ERROR_SIGNER_NOT_CREATOR
        );
    }
    ...
    inline fun assert_dispute_not_opened(offer: &Offer) {
        ...
        assert!(
            !offer.dispute_opened, 
            ERROR_DISPUTE_ALREADY_OPENED
        );
    }
    ...
    inline fun assert_singer_is_arbiter(arbiter: &signer, offer: &Offer) {
        ...
        assert!(
            signer::address_of(arbiter) == offer.arbiter, 
            ERROR_SIGNER_NOT_ARBITER
        );
    }
