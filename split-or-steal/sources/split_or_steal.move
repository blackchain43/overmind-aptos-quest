...
module overmind::split_or_steal {
    ...
    */
    fun init_module(account: &signer) {
        ...
        let (pda, cap) = account::create_resource_account(account, SEED);
        ...
        move_to(
           account,
           State {
            next_game_id: (0 as u128),
            games: simple_map::create<u128, Game>(),
            cap: cap,
            create_game_events: account::new_event_handle<CreateGameEvent>(&pda),
            submit_decision_events: account::new_event_handle<SubmitDecisionEvent>(&pda),
            reveal_decision_events: account::new_event_handle<RevealDecisionEvent>(&pda),
            conclude_game_events: account::new_event_handle<ConcludeGameEvent>(&pda),
            release_funds_after_expiration_events: account::new_event_handle<ReleaseFundsAfterExpirationEvent>(&pda)
            }
        )
    }
    ...
    */
    public entry fun create_game(
        ...
        let event_creation_timestamp_in_seconds = timestamp::now_seconds();
        let expiration_timestamp_in_seconds = EXPIRATION_TIME_IN_SECONDS + event_creation_timestamp_in_seconds;
        let game = Game {
            prize_pool_amount,
            player_one: PlayerData{
        ...
        event::emit_event<CreateGameEvent>( 
            ...
            CreateGameEvent{
                ...
                player_two_address,
                expiration_timestamp_in_seconds,
                event_creation_timestamp_in_seconds
            }
        );
    }
    ...
    */
    public entry fun submit_decision(
        ...
        let specific_game = simple_map::borrow_mut(&mut state.games, &game_id);
        check_if_player_participates_in_the_game(player, specific_game);

        // TODO: Call `check_if_player_does_not_have_a_decision_submitted` function
        check_if_player_does_not_have_a_decision_submitted(specific_game, player_address);

        // TODO: Set the player's PlayerData decision_hash and salt_hash fields to the values provided in the params
        
        if (player_address == specific_game.player_one.player_address){
            specific_game.player_one.decision_hash = option::some(decision_hash);
            specific_game.player_one.salt_hash = option::some(salt_hash);
        } else {
            specific_game.player_two.decision_hash = option::some(decision_hash);
            specific_game.player_two.salt_hash = option::some(salt_hash);
        };

        // TODO: Emit `SubmitDecisionEvent` event
        event::emit_event<SubmitDecisionEvent>( 
            &mut state.submit_decision_events,
            SubmitDecisionEvent{
                game_id,
                player_address,
                decision_hash,
    ...
    */
    public entry fun reveal_decision(
        ...
        check_if_both_players_have_a_decision_submitted(specific_game);

        // TODO: Call `make_decision` function with appropriate PlayerData instance depending on the player's address
        
        let decision = if (player_address == specific_game.player_one.player_address){
            make_decision(&mut specific_game.player_one, &salt)
        } else {
            make_decision(&mut specific_game.player_two, &salt)
        };

        // TODO: Emit `RevealDecisionEvent` event 
        event::emit_event<RevealDecisionEvent>( 
            &mut state.reveal_decision_events,
            RevealDecisionEvent{
                game_id,
                player_address,
                decision,
        ...
        if (specific_game.player_one.decision != DECISION_NOT_MADE && specific_game.player_two.decision != DECISION_NOT_MADE){
            let specific_game = *specific_game;
            ...
            if(specific_game.player_one.decision == DECISION_SPLIT && specific_game.player_two.decision == DECISION_SPLIT){
                let transfer_amount = specific_game.prize_pool_amount / 2;
                coin::transfer<AptosCoin>(resource_account, specific_game.player_one.player_address, transfer_amount);
                coin::transfer<AptosCoin>(resource_account, specific_game.player_two.player_address, transfer_amount);
            };
            if(specific_game.player_one.decision == DECISION_SPLIT && specific_game.player_two.decision == DECISION_STEAL){
                coin::transfer<AptosCoin>(resource_account, specific_game.player_two.player_address, specific_game.prize_pool_amount);
            };
            if(specific_game.player_one.decision == DECISION_STEAL && specific_game.player_two.decision == DECISION_SPLIT){
                coin::transfer<AptosCoin>(resource_account, specific_game.player_one.player_address, specific_game.prize_pool_amount);
            };
            if(specific_game.player_one.decision == DECISION_STEAL && specific_game.player_two.decision == DECISION_STEAL){
                coin::transfer<AptosCoin>(resource_account, @overmind, specific_game.prize_pool_amount);
            };
            event::emit_event<ConcludeGameEvent>( 
                &mut state.conclude_game_events,
                ConcludeGameEvent{
                    game_id,
                    player_one_decision: specific_game.player_one.decision,
                    player_two_decision: specific_game.player_two.decision,
                    prize_pool_amount: specific_game.prize_pool_amount,
                    event_creation_timestamp_in_seconds: timestamp::now_seconds() 
                }
            );
        }
    ...
    */
    public entry fun release_funds_after_expiration(_account: &signer, game_id: u128) acquires State {
        ...
        let (_, specific_game) = simple_map::remove(&mut state.games, &game_id);

        // TODO: Call `check_if_game_expired` function
        check_if_game_expired(&specific_game);
        ...
        if (specific_game.player_one.decision == DECISION_NOT_MADE && specific_game.player_two.decision == DECISION_NOT_MADE) {
            coin::transfer<AptosCoin>(resource_account, @overmind, specific_game.prize_pool_amount);
        } else if(specific_game.player_one.decision != DECISION_NOT_MADE && specific_game.player_two.decision == DECISION_NOT_MADE){
            coin::transfer<AptosCoin>(resource_account, specific_game.player_one.player_address, specific_game.prize_pool_amount);
        } else {
            coin::transfer<AptosCoin>(resource_account, specific_game.player_two.player_address, specific_game.prize_pool_amount);
        };
        ...
        event::emit_event<ReleaseFundsAfterExpirationEvent>( 
            &mut state.release_funds_after_expiration_events,
    ...
    */
    inline fun make_decision(player_data: &mut PlayerData, salt: &String): u64 {
        ...
        check_if_hash_is_correct(*option::borrow(&player_data.salt_hash), *string::bytes(salt));
        ...
        let split_decision = bcs::to_bytes(&DECISION_SPLIT);
        vector::append(&mut split_decision, *string::bytes(salt));
        let split_decision_hash = hash::sha3_256(split_decision);

        // TODO: Create a SHA3_256 hash of a steal decision from a vector containing serialized DECISION_STEAL const
        //      and bytes of the salt
        let steal_decision = bcs::to_bytes(&DECISION_STEAL);
        vector::append(&mut steal_decision, *string::bytes(salt));
        let steal_decision_hash = hash::sha3_256(steal_decision);
        ...
        let result: u64 = 0;
        if(*option::borrow(&player_data.decision_hash) == split_decision_hash){
            result = DECISION_SPLIT;
        } else if (*option::borrow(&player_data.decision_hash) == steal_decision_hash){
            result = DECISION_STEAL;
        };
        player_data.decision = result;
        result
    }
    ...
    */
    ...
    inline fun check_if_state_exists() {
        ...
        assert!(
            exists<State>(@overmind), EStateIsNotInitialized
        );
    }
    ...
    inline fun check_if_account_has_enough_apt_coins(account: &signer, amount: u64) {
        ...
        assert!(coin::balance<AptosCoin>(signer::address_of(account)) >= amount, ESignerHasInsufficientAptBalance);
    }
    ...
    inline fun check_if_player_participates_in_the_game(player: &signer, game: &Game) {
        ...
        let player_address = signer::address_of(player);
        assert!(
            player_address == game.player_one.player_address || player_address == game.player_two.player_address,
    ...
    inline fun check_if_player_does_not_have_a_decision_submitted(game: &Game, player_address: address) {
        ...
        if (player_address == game.player_one.player_address) {
            assert!(option::is_none(&game.player_one.decision_hash), EPlayerHasDecisionSubmitted);
        } else if (player_address == game.player_two.player_address) {
            assert!(option::is_none(&game.player_two.decision_hash), EPlayerHasDecisionSubmitted);
        } else {
            assert!(false, EPlayerDoesNotParticipateInTheGame);
        }
    }
