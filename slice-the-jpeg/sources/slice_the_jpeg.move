...
module overmind::slice_the_jpeg {
    ...
    fun init_module(account: &signer) {
        ...
        let (pda, pda_cap) = account::create_resource_account(account, SEED);

        // TODO: Register the resource account with the AptosCoin coin module.
        coin::register<AptosCoin>(&pda); 
        ...
        let collection_address = create_split_collection(&pda);

        // TODO: Create a new State object and send it to account
        move_to(
           account,
           State {
                signer_capability: pda_cap,
                collection_address,
                split_events: account::new_event_handle<SplitEvent>(&pda),
                redeem_events: account::new_event_handle<RedeemEvent>(&pda),
                call_events: account::new_event_handle<CallEvent>(&pda),
                exchange_events: account::new_event_handle<ExchangeEvent>(&pda)
            }
        )
    ...
    public entry fun split<NFT: key >(
        ...
        let signer_addr = signer::address_of(nf_token_owner_signer);
        let nft_object = object::address_to_object<NFT>(nf_token_address);
        check_if_account_owns_nft<NFT>(signer_addr, nft_object);
        ...
        object::transfer<NFT>(nf_token_owner_signer, nft_object, resource_account_address);
        ...
        create_split_token_as_fungible_token(
            &account::create_signer_with_capability(resource_account_signer_cap),
            //note here
            ...
            split_token_supply,
            nf_token_address,
            token::creator<NFT>(nft_object),
            token::collection_name<NFT>(nft_object),
            token::name<NFT>(nft_object)
        );
        ...
        mint_internal(split_token, signer_addr, split_token_supply);

        // TODO: Emit a new SplitEvent
        event::emit_event<SplitEvent>( 
            &mut state.split_events,
            SplitEvent {
                splitter_address: signer_addr,
    ...
    public entry fun redeem<NFT: key>(
        ...
        let state = borrow_global_mut<State>(@overmind);
        let resource_account_signer_cap = &state.signer_capability;
        let resource_account = &account::create_signer_with_capability(resource_account_signer_cap);
        let resource_account_address = account::get_signer_capability_address(resource_account_signer_cap);
        let split_token_object = object::address_to_object<SplitToken>(split_token_address);
        let nft_token_address = property_map::read_address<SplitToken>(&split_token_object, &string::utf8(PROPERTY_NAME_NFT_ADDRESS));
        check_if_nf_token_is_owned_by_account<NFT>(nft_token_address, resource_account_address);
        ...
        let split_token_supply = property_map::read_u64<SplitToken>(&split_token_object, &string::utf8(PROPERTY_NAME_MAX_SUPPLY));
        let split_token_balance = split_balance(coin_owner_address, split_token_object);
        check_if_split_token_balance_is_equal_to_supply(split_token_balance, split_token_supply);

        // TODO: Burn all of the received split tokens
        // 
        // USE: Use the burn_internal function below
        burn_internal(coin_owner, split_token_object, split_token_balance);
        ...
        let nft_object = object::address_to_object<NFT>(nft_token_address);
        object::transfer<NFT>(resource_account, nft_object, coin_owner_address);

        // TODO: Emit a new Redeem Event
        event::emit_event<RedeemEvent>( 
            &mut state.redeem_events,
            RedeemEvent {
                redeemer_address: coin_owner_address, 
                nft_address: nft_token_address
            }
        );
    ...
    public entry fun call<NFT: key>(
        ...
        check_if_split_token_balance_is_equal_to_or_greater_than_call_threshold(split_token_balance, call_threshold);
        ...
        let split_token_supply = property_map::read_u64<SplitToken>(&split_token_object, &string::utf8(PROPERTY_NAME_MAX_SUPPLY));
        let aptos_coin_balance = coin::balance<AptosCoin>(coin_owner_address);
        let caller_split_token_amount = split_token_supply - split_token_balance;
        let aptos_call_payment_amount =  caller_split_token_amount * call_price;
        check_if_aptos_coin_balance_is_equal_to_or_greater_than_call_payment(aptos_coin_balance, aptos_call_payment_amount);

        // TODO: Transfer the correct amount of AptosCoin from the coin owner to the module's
        //       resource account
        coin::transfer<AptosCoin>(coin_owner, resource_account_address, aptos_call_payment_amount);
        ...
        event::emit_event<CallEvent>( 
            ...
            CallEvent {
                caller_address: coin_owner_address, 
                nft_address: nft_token_address, 
                caller_split_token_amount,
                aptos_call_payment_amount
            }
        );
    ...
    public entry fun exchange_split_tokens_for_call_payment<NFT: key >(
        ...
        let split_token_object = object::address_to_object<SplitToken>(split_token_address);
        let nft_token_address = property_map::read_address<SplitToken>(&split_token_object, &string::utf8(PROPERTY_NAME_NFT_ADDRESS));
        check_if_nf_token_is_not_owned_by_account<NFT>(nft_token_address, resource_account_address);
        ...
        let split_token_balance = split_balance(coin_owner_address, split_token_object);
        check_if_split_balance_is_above_zero(split_token_balance);

        // TODO: Transfer the correct amount of AptosCoin from the coin owner to the module's 
        //       resource account
        let call_price = property_map::read_u64<SplitToken>(&split_token_object, &string::utf8(PROPERTY_NAME_CALL_PRICE));
        let aptos_coin_exchange_payment =  split_token_balance * call_price;
        coin::transfer<AptosCoin>(resource_account, coin_owner_address, aptos_coin_exchange_payment);
        ...
        event::emit_event<ExchangeEvent>( 
            ...
            ExchangeEvent {
                exchanger_address: coin_owner_address, 
                nft_address: nft_token_address, 
                split_token_exchange_amount: split_token_balance, 
                aptos_coin_exchange_payment
            }
        );
    ...
    fun create_split_collection(creator: &signer): address {
        ...
        collection::create_unlimited_collection(
            creator,
            string::utf8(SPLIT_COLLECTION_DESCRIPTION),
            string::utf8(SPLIT_COLLECTION_NAME),
            option::none(),
            string::utf8(SPLIT_COLLECTION_URI),
        );

        // TODO: Return the address of the newly creted token collection
        collection::create_collection_address(&signer::address_of(creator), &string::utf8(SPLIT_COLLECTION_NAME))
    ...
    fun create_split_token_as_fungible_token(
        ...
        property_map::add_typed<address>(&property_mutator_ref, string::utf8(PROPERTY_NAME_NFT_ADDRESS), nft_address);
        property_map::add_typed<u64>(&property_mutator_ref, string::utf8(PROPERTY_NAME_CALL_PRICE), call_price);
        property_map::add_typed<u64>(&property_mutator_ref, string::utf8(PROPERTY_NAME_CALL_THRESHOLD), call_threshold);
        property_map::add_typed<u64>(&property_mutator_ref, string::utf8(PROPERTY_NAME_MAX_SUPPLY), max_supply);
        property_map::add_typed<String>(&property_mutator_ref, string::utf8(PROPERTY_NAME_NFT_TOKEN_NAME), nft_token_name);
        property_map::add_typed<String>(&property_mutator_ref, string::utf8(PROPERTY_NAME_NFT_COLLECTION_NAME), nft_collection_name);
        property_map::add_typed<address>(&property_mutator_ref, string::utf8(PROPERTY_NAME_NFT_CREATOR_ADDRESS), nft_creator_address);
    ...
    public fun split_balance(account: address, split_token: Object<SplitToken>): u64 {
        ...
        primary_fungible_store::balance(account, object::convert<SplitToken, Metadata>(split_token))
    ...
    fun mint_internal(split_token_object: Object<SplitToken>, receiver: address, amount: u64) acquires SplitToken {
        ...
        //
        // HINT: https://aptos.dev/standards/aptos-token-v2/#fungible-token
        let split_token = borrow_global<SplitToken>(object::object_address(&split_token_object));
        let to_wallet = primary_fungible_store::ensure_primary_store_exists(receiver, split_token_object);
        let fa = fungible_asset::mint(&split_token.fungible_asset_mint_ref, amount);
        fungible_asset::deposit_with_ref(&split_token.fungible_asset_transfer_ref, to_wallet, fa); 
    }
    ...
    fun burn_internal(owner: &signer, split_token_object: Object<SplitToken>, amount: u64) acquires SplitToken {
        ...
        //
        // HINT: https://aptos.dev/standards/aptos-token-v2/#fungible-token
        let burn_ref = &borrow_global<SplitToken>(object::object_address(&split_token_object)).fungible_asset_burn_ref;
        let from_wallet = primary_fungible_store::ensure_primary_store_exists(signer::address_of(owner), split_token_object);
    ...
    inline fun check_if_account_owns_nft<NFT: key>(account_address: address, nf_token_object: Object<NFT>) {
       assert!(object::owns<NFT>(nf_token_object, account_address), EAccountDoesNotOwnNft);
    }
    ...
    inline fun check_if_split_token_does_not_exist(split_token_address: address) {
        assert!(!object::is_object(split_token_address), ESplitTokenAlreadyExists);
    }
    ...
    inline fun check_if_nf_token_is_owned_by_account<NFT: key>(nf_token_address: address, account_address: address) {
        let token = object::address_to_object<NFT>(nf_token_address);
        assert!(object::owns<NFT>(token, account_address), ENftIsNotOwnedByAccount);
    }
    ...
    inline fun check_if_nf_token_is_not_owned_by_account<NFT: key>(nf_token_address: address, account_address: address) {
        let token = object::address_to_object<NFT>(nf_token_address);
        assert!(!object::is_owner<NFT>(token, account_address), ENftIsOwnedByAccount);
    }
